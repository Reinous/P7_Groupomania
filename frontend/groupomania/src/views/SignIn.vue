...
export default {
	...
	methods: {
		async signUp() {
			...
			//eslint-disable-next-line
			let usernameRegex =
				/^[a-zA-Z1-9áàâäãåçéèêëíìîïñóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ\s-]{3,60}$/;
			...
			if (!this.userSignin.password.match(passwordRegex)) {
				document.querySelector('.msg-err-password').innerHTML =
					'Veuillez choisir un mot de passe valide ! Il doit contenir au moins 8 caractères dont une majuscule, une minuscule et un chiffre';
			} else {
